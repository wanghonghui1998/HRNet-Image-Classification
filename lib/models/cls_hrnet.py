...
class HighResolutionNet_cifar(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        self.conv1 = nn.Conv2d(3, start_channels, kernel_size=3, stride=2, padding=1,
                               bias=False)
        self.bn1 = nn.BatchNorm2d(24, momentum=BN_MOMENTUM)
        ...
        self.layer1 = self._make_layer(block, start_channels, num_channels, num_blocks)
    ...
    def _make_head(self, pre_stage_channels):
        ...
        final_layer = nn.Sequential(
            ...
            nn.BatchNorm2d(final_channels, momentum=BN_MOMENTUM),
            nn.ReLU(inplace=True)
        )
...
def get_cls_net_cifar(config, **kwargs):
    model = HighResolutionNet_cifar(config, **kwargs)
    model.init_weights()
    return model


if __name__ == "__main__":
    model = get_cls_net_cifar(config, **kwargs)
